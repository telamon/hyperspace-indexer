...
module.exports = async function analyzeText ({ drive, readRemoteFile, log, writeEntry, index, setPreview, seq }, file) {
  ...
  // Process beaker comments.
  if (false && file.match(/^comments\/./) && stat.metadata && stat.metadata['beaker/subject']) {
  ...
  for (const link of text.matchAll(new RegExp(MDLINK_EXP, 'g'))) {
    ...
    const key = `backlinks/${url.host}/${driveKey}_${file.replace(/\//g, '+')}`
  ...
  for (const sentence of sentences) {
    for (const token of words(sentence.replace(MDLINK_EXP, '$1'))) {
      ...
      const key = `${indexName}/${tokenPath}/${driveKey}_${file.replace(/\//g, '+')}`
      const data = {
  ...
  if (stat.size > 1024 * 4) return log('Warning encountered text >4kB what do?')
